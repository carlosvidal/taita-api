...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.headers['x-taita-subdomain']) {
      ...
      console.log('Usando subdominio del header X-Taita-Subdomain para menú:', subdomain);
    }
    // Si no hay header, intentar obtenerlo de los query params
    else if (req.query.subdomain) {
      subdomain = req.query.subdomain;
      console.log('Usando subdominio de query param para menú:', subdomain);
    }
    // Como última opción, intentar extraerlo del host
      ...
      if (host.includes('localhost') || host.includes('127.0.0.1')) {
        ...
        console.log('Desarrollo local, usando subdominio por defecto para menú:', subdomain);
      } else {
      ...
      console.log('Extraído subdominio del host para menú:', subdomain);
    }
    
    // Si aún no tenemos subdominio, usar 'demo' como último recurso
    if (!subdomain) {
      subdomain = 'demo';
      console.log('Usando subdominio por defecto (demo) para menú');
    }
